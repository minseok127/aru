...
// 업데이트 스레드
// ---------------------------------------------------
void updateThreadFunc(aru* myAru)
{
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        // tag = NULL
        aru_update(myAru, nullptr, updateOrderbookCallback, cloned);
...
// ---------------------------------------------------
void readThreadFunc_allBooks(aru* myAru)
{
    // tag 배열: book 개수만큼
    std::vector<aru_tag> tags;

    while (g_running.load(std::memory_order_relaxed)) {
        tags.clear();
        tags.resize(g_numBooks, ARU_TAG_PENDING);
